#!/usr/bin/env python3
"""Extract the Vampire Survivors Inlaid Library tilemap into runtime JSON."""
...
def build_map(repo_root: Path) -> dict:
    ...
    prefab_path = (
        assets_root / "App" / "Art" / "Tilemaps" / "Levels" / "Library" / "Library.prefab"
    )
    atlas_source = assets_root / "Texture2D" / "LibraryTexturePacked.png"
    wall_path = assets_root / "MonoBehaviour" / "Library.tmx_Walls.asset"
    obstacle_path = assets_root / "MonoBehaviour" / "Library.tmx_Obstacle.asset"
    ...
    if not selected_blocks:
        raise RuntimeError("No target tilemap layers found in Library.prefab")
    ...
    collision_rects.extend(tilemap.parse_collision_rects(obstacle_path, map_info["mapHeight"]))

    atlas_output = repo_root / "assets" / "images" / "maps" / "LibraryTexturePacked.png"
    atlas_output.parent.mkdir(parents=True, exist_ok=True)
    shutil.copyfile(atlas_source, atlas_output)

    return {
        "atlas": "assets/images/maps/LibraryTexturePacked.png",
        **map_info,
        "layers": layers,
...
def main() -> int:
    ...
    output_path = repo_root / "assets" / "Data" / "maps" / "library_map.json"
    output_path.parent.mkdir(parents=True, exist_ok=True)

    library_map = build_map(repo_root)
    output_path.write_text(json.dumps(library_map, indent=2), encoding="utf-8")

    floor = next((layer for layer in library_map["layers"] if layer["name"] == "Floor"), None)
    if library_map["mapWidth"] != 64 or library_map["mapHeight"] != 64:
        raise RuntimeError("Library map dimensions are not 64x64")
    if not floor or not floor["tiles"]:
        raise RuntimeError("Generated library map has no Floor tiles")
    if not library_map["collisionRects"]:
        raise RuntimeError("Generated library map has no collision rectangles")

    print(f"Wrote {output_path}")
    print(f"Copied atlas to {repo_root / library_map['atlas']}")
    print(f"Layers: {', '.join(layer['name'] for layer in library_map['layers'])}")
    print(f"Collision rects: {len(library_map['collisionRects'])}")
    return 0
